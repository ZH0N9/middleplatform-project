export const typeEnum={
    primary:'text-white bg-red-500 hover:bg-red-400 ',
    main:'text-white bg-zinc-900 hover:bg-zinc-800',
    info:'text-white bg-blue-500 hover:bg-blue-400'
}
export const iconTypeEnum={
    primary:{
        cls:'w-2 h-2',
        fill:'#fff'
    },
    main:{
        cls:'w-2 h-2',
        fill:'#fff',
    },
    info:{
        cls:'w-2 h-2',
        fill:'#fff'
    }
}

export const sizeEnum={
    default:'w-min-[45px] h-4 text-base px-1.5 py-1',
    small:'w-min-[60px] h-3 text-base px-1 py-0.5',
    middle:'w-min-[45px] h-4 text-base px-1.5 py-1',
    large:'w-min-[75px] h-5 text-base px-2 py-1.5',
}

export const shapeEnum={
    default:'rounded-sm',
    circle:'rounded-full',
    round:'rounded-lg',
    rectangle:''
}